    ...
    async def process_user_message(
        ...
        try:
            ...
            if use_streaming:
                ...
                # Set loading state (Slack 1.26+ feature)
                try:
                    await client.assistant_threads_setStatus(
                        channel_id=channel,
                        thread_ts=thread_ts,
                        status=f"{agent_name} is analyzing your request...",
                    )
                except Exception as e:
                    # Gracefully degrade if loading states not available
                    logger.debug("Loading state not supported", error=str(e))
                ...
                try:
                    async for stream_event in a2a_client.stream_agent(namespace, agent_name, message, session_id, user_id):
                        # Handle different event types
                        if isinstance(stream_event, TaskStatusUpdateEvent):
                            task_state = stream_event.status.state

                            # Detect input_required state (interrupt)
                            if task_state == TaskState.input_required:
                                pending_interrupt = stream_event.status
                                pending_interrupt_task_id = stream_event.task_id
                                pending_interrupt_context_id = stream_event.context_id
                                break  # Stop streaming, show approval UI

                            # Extract message text from agent
                            if stream_event.status.message:
                                msg = stream_event.status.message
                        ...
                        elif isinstance(stream_event, TaskArtifactUpdateEvent):
                            # Artifact updates REPLACE content (not append)
                            # Each artifact is a complete message unit
                            artifact_text = ""
                            for part in stream_event.artifact.parts:
                                if isinstance(part.root, TextPart):
                    ...
                    if pending_interrupt and pending_interrupt_task_id:
                    ...
                    # Clear loading state before showing final response
                    try:
                        await client.assistant_threads_setStatus(
                            channel_id=channel,
                            thread_ts=thread_ts,
