...
def register_action_handlers(app: AsyncApp, a2a_client: A2AClient) -> None:
        ...
        if "/" in agent_full_name:
            ...
            try:
                ...
                approval_parts: list[Part] = [
                    Part(root=TextPart(text="APPROVED: User approved. Proceed with the action.")),
                    Part(root=DataPart(data={"decision_type": "tool_approval", "decision": "approve"})),
                ]
    ...
    async def handle_feedback_positive(
        ack: Any,
        action: dict[str, Any],
        body: dict[str, Any],
        client: AsyncWebClient,
        ...
        from ..handlers.middleware import agent_feedback_total
        agent_feedback_total.labels(agent=agent_name, sentiment="positive").inc()
    ...
    async def handle_feedback_negative(
        ack: Any,
        action: dict[str, Any],
        body: dict[str, Any],
        client: AsyncWebClient,
        ...
        # Record feedback metric
        from ..handlers.middleware import agent_feedback_total
        agent_feedback_total.labels(agent=agent_name, sentiment="negative").inc()

        # Update message to show feedback was recorded
        await client.chat_postEphemeral(
            channel=channel,
            user=user_id,
            text="👎 Thanks for your feedback! We'll work on improving this agent.",
            thread_ts=message_ts,
        )
